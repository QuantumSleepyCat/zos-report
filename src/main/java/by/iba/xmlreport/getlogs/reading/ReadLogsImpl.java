package by.iba.xmlreport.getlogs.reading;
...
import by.iba.xmlreport.getlogs.reading.util.StatusChecker;
...
public class ReadLogsImpl implements ReadLogs{
	...
	@Override
	public void read(String appName) {
		for(StatusItem statusIt : StatusBarList.getInstance().getStatusItems().values())
		{
			...
			String name=statusIt.getAppName().toUpperCase();
			if(name.equals(appName))
			{
				System.out.println("flag");
				String logs = readLog(appName);
	...
	public String readLog(String appName)
	{
		String logs="";
